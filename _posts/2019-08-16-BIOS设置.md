...
Advance > CPU setup > Intel Hyper-Threading Technology ［enable］
...
Require Admin, Pass, when Flashing「当刷新BIOS时提示输入管理员密码」
